...
def ex1():
    ...
    model = pulp.LpProblem("MinimizationProblem_ex1", pulp.LpMinimize)
    ...
    for i, constraint in enumerate(model.constraints.values()):
        # Rounding errors are possible for floating points, thus we round to 2 d.
...
def ex2():
    ...
    y4 = pulp.LpVariable("y4", lowBound=0, cat=pulp.LpContinuous)
    y5 = pulp.LpVariable("y5", lowBound=0, cat=pulp.LpContinuous)
    y6 = pulp.LpVariable("y6", lowBound=0, cat=pulp.LpContinuous)
    ...
    # The best solution for x follows (by symmetry)
